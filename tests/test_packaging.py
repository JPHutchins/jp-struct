...
build leaves jpstruct.<abi>.so sitting there, so an installed distribution gets
shadowed by the working tree without anything failing. Only the legs that set
JPSTRUCT_REQUIRE_INSTALLED make the claim, so only they are held to it.
"""
...
pytestmark = pytest.mark.skipif(
    os.environ.get("JPSTRUCT_REQUIRE_INSTALLED") != "1",
...
def test_the_imported_module_is_the_file_the_distribution_installed():
    ...
    distribution = importlib.metadata.distribution("jp-struct")
    installed = {
        pathlib.Path(str(distribution.locate_file(entry))).resolve()
        for entry in distribution.files or ()
    }

    assert pathlib.Path(jpstruct.__file__).resolve() in installed


def test_the_import_did_not_come_from_the_working_tree():
    location = pathlib.Path(jpstruct.__file__).resolve()
